...
func constructNode(kind int, spec *y.Parser, semanticAction bool, src string, clist []string, ids map[string]bool, rule *y.Rule) string {
	sym := rule.Sym
	caseName := ""
	for _, d := range ruleDirectives[rule] {
		if d.cmd == "case" {
	...
	default:
		...
		m := nodeFieldMap0(sym)
		if !ignored {
			if (kind != 0 || caseName != "") && !isOpt(sym) {
				k := interface{}(kind)
				if caseName != "" {
					k = caseName
				}
				a = append(a, fmt.Sprintf("\t%v: %v,", *oKind, k))
			}
		}
		if !ignored {
			for i, c := range clist {
...
func symDocs(sym *y.Symbol) string {
	...
	for i := 1; i < len(a); i++ {
		k := interface{}(i - 1)
		for _, d := range ruleDirectives[sym.Rules[i-1]] {
			if d.cmd == "case" {
				k = nm + d.arg
				break
			}
		...
		s := a[i]
		s += fmt.Sprintf("%s// %s %v", strings.Repeat(" ", w+2-len(s)), *oKind, k)
		a[i] = s
	}
